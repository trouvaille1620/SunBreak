...
from QT_tools_code.B_Biaotu.Biaotu_tools import Via_coco2normal_coco
...
file_path = r'QT_style/B_Biaotu/via_coco2normal_coco_QT.json'
style_dict = json.load(open(file_path, 'r', encoding='utf-8'))


class Via_coco2normal_coco_QT_Demo(QtWidgets.QMainWindow):
    def __init__(self):
        super(Via_coco2normal_coco_QT_Demo, self).__init__()
        self.worker = Via_coco2normal_coco()
        self.setup_ui()
    ...
    def setup_ui(self):
        ...
        self.output_path.setPlaceholderText(("修正后的文件名为via_export_coco_normal.json"))
    ...
    def click_handler(self):
        ...
        if format_right:
                    ...
                    self.log.setText(str(">>开始修正>>"))
                    self.log.append("————————————————")
                    self.log.append(str(">>修正中，请稍等...>>"))
                    self.log.append("————————————————")
                    QApplication.processEvents()  # 刷新界面
