...
from QT_tools_code.C_Biaozhutongji.Biaozhutongji_tools import Statistics_project
...
file_path = r'QT_style/C_Biaozhutongji/statistics_project_QT.json'
style_dict = json.load(open(file_path, 'r', encoding='utf-8'))


class Statistics_project_QT_Demo(QtWidgets.QMainWindow):
    def __init__(self):
        super(Statistics_project_QT_Demo, self).__init__()
        self.worker = Statistics_project()
        self.setup_ui()
    ...
    def setup_ui(self):
        ...
        self.input_path.setPlaceholderText(("选择project.json文件"))
        self.input_path.textChanged.connect(self.set_input_path)
        self.path = None
        self.input_path.setObjectName("input_path")

        # 是否保存结果至label_num.xlsx
    ...
    def click_handler(self):
        ...
        if format_right:
            ...
            self.log.setText(str(">>开始统计>>"))
            self.log.append("————————————————")
            self.log.append(str(">>统计中，请稍等...>>"))
            self.log.append("————————————————")
            QApplication.processEvents()  # 刷新界面
            if self.rb11.isChecked() or self.rb12.isChecked():
                save_xlsx = self.rb11.isChecked()
            else:
                save_xlsx = False
            self.worker.filter_path = str(self.path)
            self.worker.save_xlsx = save_xlsx
            self.worker.start()
