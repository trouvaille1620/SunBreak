...
from QT_tools_code.C_Biaozhutongji.Biaozhutongji_tools import No_label_check_project
...
file_path = r'QT_style/C_Biaozhutongji/no_label_check_project_QT.json'
style_dict = json.load(open(file_path, 'r', encoding='utf-8'))


class No_label_check_project_QT_Demo(QtWidgets.QMainWindow):
    def __init__(self):
        super(No_label_check_project_QT_Demo, self).__init__()
        self.worker = No_label_check_project()
        self.setup_ui()
    ...
    def setup_ui(self):
        ...
        self.output_path.setPlaceholderText(("保存文件名为no_label_check.txt"))
    ...
    def click_handler(self):
        ...
        if format_right:
            ...
            self.log.setText(str(">>开始检查>>"))
            self.log.append("————————————————")
            self.log.append(str(">>检查中，请稍等...>>"))
            self.log.append("————————————————")
            QApplication.processEvents()  # 刷新界面
            self.worker.filter_path = str(self.path)
            self.worker.save_path0 = str(self.path1)
            self.worker.start()
