...
from QT_tools_code.D_Dianjian.Dianjian_tools import Extract_label_project
...
file_path = r'QT_style/D_Dianjian/extract_label_project_QT.json'
style_dict = json.load(open(file_path, 'r', encoding='utf-8'))


class Extract_label_project_QT_Demo(QtWidgets.QMainWindow):
    def __init__(self):
        super(Extract_label_project_QT_Demo, self).__init__()
        self.worker = Extract_label_project()
        self.setup_ui()
    ...
    def setup_ui(self):
        ...
        # 输入待删除的标签
        self.extract_label = Normal_Qlabel_Style(self.base_widget, **style_dict['Extract_label_Style'])

        self.labellist = Normal_QLineEdit_Style(self, **style_dict['Labellist_LineEdit_Style'])
        self.labellist.setPlaceholderText(("例：hd, kp, ... "))
