...
from ..D_Dianjian import extract_label_QT, img_name2xlsx_QT, Extract_label_project_QT, build_blank_project_model_QT, merge_project_by_label_QT
...
class Dianjian_window_StackWidget(QWidget):

    def __init__(self):
        super(Dianjian_window_StackWidget, self).__init__()
        self.setup_ui()
    ...
    def setup_ui(self):
        ...
        list_str = ['按类别拆分检出数据', '生成图片名到xlsx', '按类别提取project', '为图片建立空project', '添加类别标注至project']
        tools_str = [getattr(extract_label_QT, "Extract_label_QT_Demo"),
                     getattr(img_name2xlsx_QT, "Img_name2xlsx_QT_Demo"),
                     getattr(Extract_label_project_QT, "Extract_label_project_QT_Demo"),
                     getattr(build_blank_project_model_QT, "Build_blank_project_model_Demo"),
                     getattr(merge_project_by_label_QT, "Merge_project_by_label_QT_Demo"),
                     ]
        for i in range(5):
