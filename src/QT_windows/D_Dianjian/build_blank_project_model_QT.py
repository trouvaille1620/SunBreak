...
from QT_tools_code.D_Dianjian.Dianjian_tools import Build_blank_project_model
...
file_path = r'QT_style/D_Dianjian/build_blank_project_model_QT.json'
style_dict = json.load(open(file_path, 'r', encoding='utf-8'))


class Build_blank_project_model_Demo(QtWidgets.QMainWindow):
    def __init__(self):
        super(Build_blank_project_model_Demo, self).__init__()
        self.worker = Build_blank_project_model()
        self.setup_ui()
    ...
    def click_handler(self):
        ...
        if format_right:
            ...
            self.log.setText(str(">>开始建立>>"))
            self.log.append("————————————————")
            self.log.append(str(">>建立中，请稍等...>>"))
            self.log.append("————————————————")
            QApplication.processEvents()  # 刷新界面
